...
void makeGradientImage(Mat* mat);
Mat applyLaplacianZeroX(IplImage* img);
void applyLaplacFilter(IplImage* img);
void computeZeroX(Mat* mat);
	...
	while(key != 'q') {
		switch(key) {
			...
			case 'i':
				...
				imshow("ELEC536_Assignment1", applyLaplacianZeroX(img));
				break;

			case 'j':
				cout << "Zero crossing" << endl;
				mat = imread("blocks.jpg", CV_LOAD_IMAGE_GRAYSCALE);
				makeGradientImage(&mat);
				imshow("ELEC536_Assignment1", mat);
				cvWaitKey();
				//mat = (Mat) img;
				computeZeroX(&mat);
				imshow("ELEC536_Assignment1", mat);
...
void makeGradientImage(Mat* mat) {
	...
	for(int i = 1; i < mat->cols - 1; i++) {
		for(int j=1; j < mat->rows - 1;j++) {
			//magnitude along x:
			a = (int)mat->at<uchar>(i,j-1);
			b = (int)mat->at<uchar>(i,j+1);
	        xMag = -0.5 * a + 0.5 * b;

			//magnitude along y:
			c = (int)mat->at<uchar>(i-1,j);
			d = (int)mat->at<uchar>(i+1,j);
			yMag = -0.5*c + 0.5 * d;

			orientation = atan2(yMag, xMag);
			totalMag = sqrt(xMag*xMag + yMag*yMag);
			mat->at<uchar>(i,j) = (orientation + PI) * 90;
			//compare to threshold to decide if this is an edge
//			if(totalMag < gradMagThreshold) {
//				mat->at<uchar>(i,j) = 220;
//			} else {
//				mat->at<uchar>(i,j) = orientation + PI;
//			}
		}
	}
}
/**
 * This method traverses the image (mat) with a 2x2 matrix. If there is a change in the sign inside the matrix the
...
void computeZeroX(Mat* mat) {
	for(int i = 0; i < mat->cols - 1; i++) {
		for(int j=0; j < mat->rows - 1;j++) {
			int a = (int)(mat->at<uchar>(i,j) - PI);
			int b = (int)(mat->at<uchar>(i+1,j) - PI);
			int c = (int)(mat->at<uchar>(i,j+1) - PI);
			int d = (int)(mat->at<uchar>(i+1,j+1) - PI);
			int absSum = abs(a+b+c+d);
			if( absSum != (abs(a) + abs(b) + abs(c) + abs(d))) {
				//there is a change in sign. mark (i,j) as edge
				mat->at<uchar>(i,j) = 240;
			} else {
				//It is background
				mat->at<uchar>(i,j) = 10;
			}
